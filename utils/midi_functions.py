...
def get_note_training_set(prev_notes=50):
    ...
    for file in midi_files:
        ...
        while(i + prev_notes + 1 < len(notes)):
            if True:  #notes[i + prev_notes] > 25 and notes[i + prev_notes] < 101:
    ...
    np.savetxt('notes/' + str(prev_notes) + 'X.txt',X,fmt='%i')
    np.savetxt('notes/' + str(prev_notes) + 'Y.txt',Y,fmt='%i')
    
    return (X,Y)

def get_hijacked_note_training_set(prev_notes=50):
    midi_files = glob.glob("./clementi_songs/*.mid")
    X = []
    Y = []
    for file in midi_files:
        notes = get_notes(path=file)
        ...
        while(i + prev_notes < len(notes)):
            if True: #notes[i + prev_notes - 1] > 25 or notes[i + prev_notes - 1] < 101:
                X.append(notes[i:i + prev_notes])
    ...
    np.savetxt('notes/' + str(prev_notes) + 'clementiHX.txt',X,fmt='%i')
    np.savetxt('notes/' + str(prev_notes) + 'clementiHY.txt',Y,fmt='%i')
    
    return (X,Y)

def create_midi_from_notes(notes, title, on=512, off=0, vel_on=90, vel_off=0):
    mid = mido.MidiFile()
    track = mido.MidiTrack()
    mid.tracks.append(track)
